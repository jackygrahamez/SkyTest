...
//		debug("Sender   BookCode:"+bookCode+" "+percent);
	...
	public void donwloadCover(String url,String fileName) {
		String targetFile = new String(SkySetting.getStorageDirectory() + "/covers/"+fileName);
...
//				Log.e("donwloadCover error: ", e.getMessage().toString());
		...
		public void onReceive(Context context, Intent intent) {
            ...
            if (c.moveToFirst()) {
                ...
                if (DownloadManager.STATUS_SUCCESSFUL == c.getInt(columnIndex)) {
                    ...
                    String sourceFile = uriString.substring(7);
        			...
        			bi.bookCode = bookCode;
        			bi.fileSize = bytes_total;
        			bi.downSize = bytes_total;
        			bi.isDownloaded = false;
        			app.sd.updateBook(bi);
                    String targetFile = new String(SkySetting.getStorageDirectory() + "/downloads/"+app.sd.getFileNameByBookCode(bookCode));
	...
	class UnzipHandler implements Observer {
		...
		public void update(Observable observable, Object data) {
			...
			int bookCode = app.sd.getBookCodeByFileName(fileName);
			String coverPath = getCoverPathByBookCode(bookCode);
			...
			final BookInformation bi;
			if (!coverFile.exists()) {
				bi = new BookInformation(fileName,baseDirectory,contentListener,coverPath);
			}else {
				bi = new BookInformation(fileName,baseDirectory,contentListener);
			}	        	
			bi.bookCode = bookCode;
			bi.fileSize = -1;
			bi.downSize = -1;
			...
			(new Handler()).postDelayed(new Runnable() {
				public void run() {
					reloadBookInformation(bi.bookCode);
				}
			},500);
			
			String targetDir = new String(SkySetting.getStorageDirectory() + "/books/" + fileName);
			targetDir = SkyUtility.removeExtention(targetDir);
			String filePath = new String(SkySetting.getStorageDirectory() + "/downloads/");
		    SkyUtility.moveFile(filePath+fileName,targetDir+"/"+fileName);
		}		
	}
	
	public void unzipBook(int bookCode) {
	    String fileName = app.sd.getFileNameByBookCode(bookCode);
		String targetDir = new String(SkySetting.getStorageDirectory() + "/books/" + fileName);
		targetDir = SkyUtility.removeExtention(targetDir);
